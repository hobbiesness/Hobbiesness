...
EXIST_SCRIPT=$INSTALLDIR/eXist/ctl.sh
...
help() {
	...
	if test -x $EXIST_SCRIPT; then	
	    echo "       $0 (start|stop|restart|status) eXist"
	fi
...
if [ "x$1" = "xhelp" ] || [ "x$1" = "x" ]; then
    ...
    if [ "x$2" = "xmysql" ]; then
        ...
        if test -x $TOMCAT_SCRIPT; then
            sudo -u tomcat $TOMCAT_SCRIPT start  
            TOMCAT_ERROR=$?
        fi
    ...
    elif [ "x$2" = "xeXist" ]; then
        if test -x $EXIST_SCRIPT; then
            sudo -u exist $EXIST_SCRIPT start  
            EXIST_ERROR=$?
        fi
    ...
    if [ "x$2" = "xmysql" ]; then
        ...
        if test -x $TOMCAT_SCRIPT; then
            sudo -u tomcat $TOMCAT_SCRIPT stop
            TOMCAT_ERROR=$?
        fi
    elif [ "x$2" = "xsling" ]; then
        if test -x $SLING_SCRIPT; then
        ...
        if test -x $EXIST_SCRIPT; then
            sudo -u exist $EXIST_SCRIPT stop
            EXIST_ERROR=$?
        fi
    elif [ "x$2" = "xresin" ]; then
        if test -x $RESIN_SCRIPT; then
    ...
    if [ "x$2" = "xmysql" ]; then
        ...
        if test -x $TOMCAT_SCRIPT; then
            sudo -u tomcat $TOMCAT_SCRIPT status
        fi
    elif [ "x$2" = "xsling" ]; then
        if test -x $SLING_SCRIPT; then
        ...
        if test -x $EXIST_SCRIPT; then
            sudo -u exist $EXIST_SCRIPT status
        fi
    elif [ "x$2" = "xresin" ]; then
        if test -x $RESIN_SCRIPT; then
            $RESIN_SCRIPT status
    ...
    if test -x $TOMCAT_SCRIPT; then
        sudo -u tomcat $TOMCAT_SCRIPT cleanpid
    fi
    if test -x $SLING_SCRIPT; then
    ...
    if test -x $EXIST_SCRIPT; then
        sudo -u exist $EXIST_SCRIPT cleanpid
    fi
    if test -x $MYSQL_SCRIPT; then
...
# Checking for errors
for e in $APACHE_ERROR $MYSQL_ERROR $SUBVERSION_ERROR $TOMCAT_ERROR $SLING_ERROR $SOLR_ERROR $EXIST_ERROR $RESIN_ERROR $MEMCACHED_ERROR $INGRES_ERROR $OPENOFFICE_ERROR $LUCENE_ERROR $ZOPE_ERROR $POSTGRESQL_ERROR $THIRD_ERROR $NAGIOS_ERROR $RABBITMQ_ERROR $JETTY_ERROR $JBOSS_ERROR $HYPERSONIC_ERROR; do
    if [ $e -gt 0 ]; then
        ERROR=$e
    fi
done
